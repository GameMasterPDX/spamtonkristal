local SpamShop, super = Class(Shop,  "spamshop")

function SpamShop:init()
    super:init(self)
    ...
    self.encounter_text = "* HEY EVERY     !    * IT'S ME, SPAMTON G.                 SPAMTON!"
    self.shop_text = "* HEY EVERY     !    * IT'S ME, SPAMTON G.                 SPAMTON!"
    ...
    self.talk_text = "LET'S MAKE/nA DEAL"
    ...
    self:registerTalk("OUR DEAL")
    self:registerTalk("ABOUT YOU")
    self:registerTalk("KNIGHT")
    self:registerTalk("FEAR")

    self:registerTalkAfter("Myself", 1)
    self:registerTalkAfter("Why Am I Here", 2)

    --[[self.shopkeeper:setActor("shopkeepers/spamton")
    self.shopkeeper.sprite:setPosition(-24, 12)
    self.shopkeeper.slide = true

    self.background = "ui/shop/bg_spam"]]

    self.background = nil
end

function SpamShop:startTalk(talk)
    if talk == "OUR DEAL" then
        self:startDialogue({
            "* KRIS!!! YOU [Little Sponge]! I/nKNEW YOU'D COME HERE [[On A Saturday Night]]!",
